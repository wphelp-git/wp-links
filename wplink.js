...
(function($) {
	...
	wpLink = {
		...
		init: function() {
			...
			inputs.relSponsored = $('#link-sponsored-checkbox'); // (new-mod)
			inputs.relUgc = $('#link-ugc-checkbox'); // (new-mod)
			...
			// Display 'hint' message when search field or 'query-results' box are
			// focused
		...
		refresh: function() {
			...
			// Load the most recent results if this is the first time opening the
			// panel.
		...
		hasSelectedText: function(linkNode) {
			...
			if (/</.test(html) &&
					(!/^<a [^>]+>[^<]+<\/a>$/.test(html) || html.indexOf('href=') ===
							-1)) {
				return false;
			}
		...
		mceRefresh: function() {
			...
			if (linkNode) {
				...
				// Set rel="sponsored" (mod)
				if ('sponsored' == ed.dom.getAttrib(e, 'rel')) {
					inputs.relSponsored.prop('checked', true);
				}

				// Set rel="ugc" (mod)
				if ('ugc' == ed.dom.getAttrib(e, 'rel')) {
					inputs.relUgc.prop('checked', true);
				}
		...
		setDefaultValues: function() {
			...
			if (this.isMCE()) {
				...
				selection = this.textarea.value.substring(this.textarea.selectionStart,
						this.textarea.selectionEnd);
			}
		...
		keydown: function(event) {
			...
			if (key.ESCAPE === event.keyCode) {
				...
				// wp-link-submit must always be the last focusable element in the
				// dialog. following focusable elements will be skipped on keyboard
				// navigation.
	...
	$.extend(River.prototype, {
		...
		next: function() {
			...
			var to = this.selected ? this.selected.next('li') : $(
		...
		process: function(results, params) {
			...
			if (!results) {
				...
				$.each(results, function() {
					...
					list += '<input type="hidden" class="item-permalink" value="' +
							this.permalink + '" />';
		...
		maybeLoad: function() {
			...
			if (!this.query.ready() || bottom < this.contentHeight.height() -
			...
			setTimeout(function() {
				...
				if (!self.query.ready() || newBottom < self.contentHeight.height() -
						wpLink.riverBottomThreshold) {
					return;
				}
